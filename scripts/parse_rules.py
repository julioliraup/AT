...
def _load_nrd_domains(domains_file=None, explicit_nrd_path=None):
    """Load suspicious NRD domains from nrd_suspicious_domains.txt.

    Return a set of lowercase domain strings.
    """
    candidates = []
    if explicit_nrd_path:
        candidates.append(Path(explicit_nrd_path))
    if domains_file:
        candidates.append(Path(domains_file).parent / 'nrd_suspicious_domains.txt')
    candidates.extend([
        Path('rules/nrd_suspicious_domains.txt'),
        Path('nrd_suspicious_domains.txt'),
    ])
    ...
    for p in candidates:
        if p.is_file():
            try:
                ...
                domains = {line.strip().lower() for line in lines if line.strip() and not line.strip().startswith('#')}
                print(f'[nrd] loaded {len(domains)} ATI NRD domains from {p}', file=sys.stderr)
                return domains
            except Exception as exc:
                print(f'[nrd] error reading {p}: {exc}', file=sys.stderr)
...
def enrich_dns(obj, domains_file, explicit_nrd_path=None):
    ...
    obj['dns_feed'] = {
        ...
        'ati_count':      len(nrd_domains),
    }
    return obj


def build_dns_tls_sids(domains_file, explicit_nrd_path=None):
    ...
    for meta in FEED_SIDS:
        record = {
            ...
            'dns_feed': {
                ...
                'top_tlds':      top_tlds,
                'domains':       domains,
                'ati_domains':    nrd_list,
                'ati_count':      len(nrd_list),
            },
        }
...
def _load_ips(ips_file):
    """Load plain-text IP list (one IPv4 per line).

    Return a sorted list of non-empty, non-comment strings.
    """
    path = Path(ips_file)
    if not path.is_file():
        return []
...
def build_ip_sid(ips_file):
    """Generate web/db/sid/6000002.json from *ips_file* (plain-text, one IPv4/line).

    Mirrors the dns/tls pattern: a single self-contained record that holds
    the full IP list under ``ip_feed.ips``.  The count is surfaced in
    index.json via ``ip_feed.ips_count``.
    """
    ips = _load_ips(ips_file)
    record = {
        'sid':        6000002,
        'protocol':   'ip',
        'msg':        'AT IP dataset - Phishing',
        'classtype':  'social-engineering',
        'rev':        1,
        'action':     'alert',
        'severity':   'high',
...
def main():
    ...
    nrd_path = sys.argv[3] if len(sys.argv) > 3 else None
